...
stCliente cargaUnCliente ()
{
    ...
    printf("INGRESE ESTADO DEL CLIENTE (0 SI EST¡ ACTIVO - 1 SI ESTA ELIMINADO).......:");
    scanf("%d", &c.eliminado);
    getchar();
    return c;
}
...
void buscaClientes(char archClientes[])
{
    ...
    if (p != NULL)   // Si se encontrÛ el cliente reliza las siguientes lineas
    {
    ...
    if(archi)
    {
        while(!encontrado && fread(&c, sizeof(stCliente), 1, archi)>0)
        {
            if(strcmp(c.dni, dni) == 0) // Si se encontro el cliente
            {
                ...
                printf("øQUE CAMPO QUIERES MODIFICAR?\n");

                printf("1. NOMBRE\n2. APELLIDO\n3. EMAIL\n4. TELEFONO\n5. CALLE\n6. NUMERO\n7. LOCALIDAD\n8. PROVINCIA\n9. C.POSTAL\n");
                int opcion;
                scanf("%d", &opcion);// Elegimos una opciÛn que queremos modificar
                getchar(); // Limpia el buffer de entrada
...
void checkPassword(char valid_password[3][10])
{
    ...
    while (!encontrado && intentos < 3)
    {
        ...
        // Verifica si la contraseÒa ingresada es v·lida
        for (i = 0; i < 3; i++)
        {
            if (strcmp(login, valid_password[i]) == 0)
            {
                encontrado = 1;
                menu();  // Si la contraseÒa es v·lida, llama a la funciÛn menu
            }
        }
        if (!encontrado)
        {
